      ...
      //  string ip = Dns.GetHostByName(hostName).AddressList[0].ToString();
		...
		private void dgEvents_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
		{
			...
			if (dgEvents.Columns[e.ColumnIndex].Name == "TypeImage")
			{
				...
				if (objType != null)
				{
					...
					{
						e.Value = SmoothyInterface.Properties.Resources.information;
					}
					else if (strType == "Warning")
					{
						e.Value = SmoothyInterface.Properties.Resources.warning;
					}
					else if (strType == "Error")
					{
						e.Value = SmoothyInterface.Properties.Resources.cancel;
					}
					else if (strType == "FailureAudit")
					{
						e.Value = SmoothyInterface.Properties.Resources.silk_lock;
					}
					else if (strType == "SuccessAudit")
					{
						e.Value = SmoothyInterface.Properties.Resources.key;
					}
				}
			}
		}
        ...
        private void button1_Click(object sender, EventArgs e)
        {
         ...
         //   xlWorkSheet = (Excel.Worksheet)xlWorkBook.Worksheets.get_Item(1);
            ...
            for (i = 0; i <= dgEvents.RowCount  - 1; i++)
            {
                for (j = 0; j <= dgEvents.ColumnCount  - 1; j++)
                {
                    DataGridViewCell cell = dgEvents[j, i];
                    xlWorkSheet.Cells[i + 1, j + 1] = cell.Value;
                }
            }

            xlWorkBook.SaveAs(@"E:\lbu.xls", Excel.XlFileFormat.xlWorkbookNormal, misValue, misValue, misValue, misValue, Excel.XlSaveAsAccessMode.xlExclusive, misValue, misValue, misValue, misValue, misValue);
